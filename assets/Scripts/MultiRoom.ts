...
export class MultiRoom extends Component {
        ...
        this.roomRef.once(
          ...
          (snapshot) => {
            ...
            if (userCnt == 0) this.roomRef.remove();
              ...
              this.roomRef.update({ userCnt: userCnt, users: this.userList });
          },
          (e) => {
            console.log(e);
          }
        );
        this.returnButton.node.getComponent(AudioSource).volume =
        ...
        try {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            const userRef = firebase
              .database()
              .ref("users/" + this.userList[i + 1]);
            userRef.once("value", (snapshot) => {
              try {
                if (!snapshot.exists()) return;
                // console.log(snapshot.val());
                const user = snapshot.val();
                this.playerFrameList[i]
                  .getChildByName("name")
                  .getComponent(Label).string = user.name;
                this.playerFrameList[i]
                  .getChildByName("KD")
                  .getComponent(Label).string =
                  user.death == 0 ? "NA" : (user.kill / user.death).toString();
              } catch (e) {
                console.log(e);
              }
            });
  ...
  onBagClick() {
    ...
    this.scheduleOnce(() => {
      ...
      setupSelect.getChildByName("cancel").on(
        Node.EventType.MOUSE_UP,
        () => {
          setupSelect.destroy();
        },
  ...
  onMapClick() {
    ...
    this.mapButton.node.getComponent(AudioSource).volume =
      Setting.EffectVolume * 2;
    ...
    // button function setup
    mapSelect.getChildByName("cancel").on(
      Node.EventType.MOUSE_UP,
      () => {
    ...
    mapSelect.getChildByName("map1").on(
      Node.EventType.MOUSE_UP,
      () => {
    ...
    mapSelect.getChildByName("map2").on(
      Node.EventType.MOUSE_UP,
      () => {
    ...
    mapSelect.getChildByName("random").on(
      Node.EventType.MOUSE_UP,
      () => {
        console.log("Randommmmmmmm");
        this.roomRef.update({
          map: mapNames[Math.floor(Math.random() * this.mapCnt)],
        });
        mapSelect.destroy();
      },
      this
